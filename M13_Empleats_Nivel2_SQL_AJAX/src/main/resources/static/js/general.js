		...
		xhr.open('get', 'http://localhost:8080/api/ajaxrest');
		xhr.send();
		...
		function cosTablaEmpHtml(valor){
			$('#tbLlistatEmp').append(
					$(								
							...
							"</td>"	+	
							"<td>" + 
							"<a id=" + valor.idEmpleat + " width='106' href='#'> EDITAR [" + valor.idEmpleat + "]" +
							"</a>" +
							"</td>" +									
						"</tr>"								
					)	
		...
		$(document).ready(function(){
			...
			$('#infoSQL').on('click',function(){
				...
				if ($('#infoSQL').val() == 'Info SQL'){
					$('#infoSQL').val('Amagar SQL');
					$('#notes').text("SQL Script visible");
				}else{
					$('#infoSQL').val('Info SQL');
